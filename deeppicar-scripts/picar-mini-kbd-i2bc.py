...
parser.add_argument("-n", "--ncpu", help="number of cores to use.", type=int, default=0)
...
if use_dnn == True:
    ...
    def create_dave(input_shape):
        """Copy paste from jupyter notebooks (TF2)"""
        ...
        dave_2_model.add(tf.keras.layers.BatchNormalization( # tf.keras.layers.LayerNormalization(
        ...
        for n_neurons in [100, 50, 4]:
            dave_2_model.add(tf.keras.layers.Dense(
                units=n_neurons,
                activation=(tf.nn.tanh if n_neurons == 4 else tf.nn.relu)
            ))
	
	dave_2_model.add(tf.keras.layers.Reshape((4,1)))

        # Compiling the model to see the initial structure
        dave_2_model.compile(
            optimizer='adam',
            loss=['mse'],  # Bezier loss function not installed in DeepPicar-v2
            metrics=['mse']
        )
    
        return dave_2_model

    model = create_dave((66,200,3))
    # model.summary()
    model.load_weights("../EECS-753-Project/models/cnn-image-to-curve.h5", by_name=True)
    
    print ("Done..")

    # Outer loop parameters
    t_start = 0. # start time window
    t_window = 0.5  # 1/2 second (2 Hz)
    n_pts = 10  # ratio of interpolations so CNN frames
    n_counter = 0  # counter for updating Bezier curve
    poles = np.zeros((4,))  # cubic
...
while True:
    ...
    if use_dnn == True:
        # Update poles if necessary
	if n_counter == 0:
            # 1. machine input
            img = preprocess.preprocess(frame)
            poles = model.predict(img.reshape((1,) + img.shape))[0,:]
            t_start = ts

        # Calculate the non-dimensional parameter -> angle
        t = (ts - t_start)/t_window
        angle = np.sum(np.array([1, 3, 3, 1])*np.array([
            (1 - t)** 3,
            t*((1 - t)** 2),
            (t ** 2)*(1 - t),
            t**3,
        ]))*1.5*np.pi/6  # 45 to 30 degree range
        car_angle = 0
